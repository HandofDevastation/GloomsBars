...
-- ---------------------------------------------------------------------------
-- DEV PROTOTYPE (session 10, NEXT #2): the rotating shine-chase. N glowing COMETS
-- (Media/art/shine.png) spun over each button, clipped by the shape's RIM MASK
-- (Media/art/hand/<key>-rim.png) so each bright point chases the silhouette's outline.
-- Comets are evenly PHASE-SPACED (360/N apart) and driven by ONE OnUpdate via
-- SetRotation — so COUNT / SPEED / DIRECTION / COLOUR are all live (no art regen);
-- only the point+tail SHAPE stays baked. `/gb shine <1-4|colour|off>` is the playground
-- to settle the look before wiring it to the per-trigger toggle + Config controls.
-- ---------------------------------------------------------------------------
local shines = {}   -- [btn] = { frame, mask, texs = {}, masked = {}, n }
local SHINE_TEX = GB.MEDIA .. "art\\shine.png"
local shineSpeed = 2 * math.pi / 2.6   -- rad/sec (2.6s per revolution) — live
local shineAngle, shineDir = 0, 1        -- live rotation phase + direction (±1)

local shineDriver = CreateFrame("Frame"); shineDriver:Hide()
shineDriver:SetScript("OnUpdate", function(_, dt)
  shineAngle = (shineAngle + dt * shineSpeed * shineDir) % (2 * math.pi)
  for _, sh in pairs(shines) do
    local n = sh.n or 0
    if n > 0 then
      local step = 2 * math.pi / n
      for i = 1, n do
        if sh.texs[i] then sh.texs[i]:SetRotation(shineAngle + (i - 1) * step) end
      end
    end
  end
end)

local function getShine(btn)
  local sh = shines[btn]
  if not sh then
    local frame = CreateFrame("Frame", nil, btn)
    frame:SetFrameLevel(btn:GetFrameLevel() + 3)   -- above the plate/glow, below text (+4)
    sh = { frame = frame, mask = frame:CreateMaskTexture(), texs = {}, masked = {}, n = 0 }
    shines[btn] = sh
  end
  return sh
end

-- Show N comets on a button, evenly phase-spaced, masked to the rim (mask attached one
-- frame late — AddMaskTexture fails on a never-rendered texture, API-NOTES §2).
local function showComets(sh, n, icon, key, color)
  local s = 1.6 * math.max(icon:GetWidth(), icon:GetHeight())   -- oversize so the spin covers the rim
  if GB.Skin and GB.Skin.AnchorHandGrown then GB.Skin:AnchorHandGrown(sh.mask, icon, 0) end
  sh.mask:SetTexture(GB:HandAsset(key, "rim"), "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
  for i = 1, n do
    local tex = sh.texs[i]
    if not tex then
      tex = sh.frame:CreateTexture(nil, "OVERLAY")
    ...
    -- The tail is baked on one side of the head, so mirror the comet horizontally when
    -- the spin reverses → the tail always trails BEHIND the head, never leads it.
    if shineDir < 0 then tex:SetTexCoord(1, 0, 0, 1) else tex:SetTexCoord(0, 1, 0, 1) end
    tex:Show()
  end
  for i = n + 1, #sh.texs do sh.texs[i]:Hide() end
  sh.n = n
  C_Timer.After(0, function()
    for i = 1, n do
      local tex = sh.texs[i]
      if tex and tex:IsShown() and not sh.masked[i] then tex:AddMaskTexture(sh.mask); sh.masked[i] = true end
    end
  end)
end

-- Dev playground: n comets (0 = off) in `color` ({r,g,b}). speed = secs/rev, dir = ±1.
function Glows:ShinePreview(n, color, speed, dir)
  self:Init()
  n = n or 0
  color = color or { 1, 1, 1 }
  if speed and speed > 0 then shineSpeed = 2 * math.pi / speed end
  if dir then shineDir = dir >= 0 and 1 or -1 end
  GB:ForEachButton(function(btn)
    if not isOurs(btn) then return end
    local sh = getShine(btn)
    local icon, key = btn.icon or btn.Icon, GB.db and GB.db.handShape
    if n > 0 and icon and key then
      showComets(sh, n, icon, key, color); sh.frame:Show()
    else
      sh.n = 0; sh.frame:Hide()
    end
  end)
  if n > 0 then shineDriver:Show() else shineDriver:Hide() end
  return n
end
